...
const systemPrompt = `You are a warm, encouraging ADHD coach named Spark. Break a big overwhelming goal into micro-tasks that feel almost TOO easy to start.
...
- Include a short coach_note (1 sentence)
...
export default function Unstuck() {
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      setPhFade(false);
      setTimeout(() => { setPlaceholderIdx(i => (i + 1) % placeholders.length); setPhFade(true); }, 350);
    }, 2800);
    ...
    if (allDone && result && !completionSaved.current) {
      ...
      setAppData(prev => {
        ...
        const newEntry = {
          ...
          date: today,
          goal: goal,
          tasks: result.tasks.map(t => ({ ...t, done: !!checked[t.id] })),
  ...
  const decompose = async () => {
	...
	try {
    // UPDATED: Using Google's Gemini API endpoint
	 const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-3-flash:generateContent?key=${YOUR_API_KEY}`, {
        method: "POST",
      ...
      // Gemini returns the text in a slightly different path than Claude
  ...
  const reset = () => { setResult(null); setGoal(""); setChecked({}); completionSaved.current = false; };

  const streakEmoji = n => n >= 7 ? "🔥" : n >= 3 ? "⚡" : "✦";
  ...
  return (
    <div style={S.root}>
      <style>{`
        ...
        @keyframes celebrate { 0%{background:rgba(255,180,60,0.0)} 40%{background:rgba(255,180,60,0.12)} 100%{background:rgba(255,180,60,0.0)} }
        .task-card:hover { border-color: rgba(255,180,60,0.3) !important; }
        .tab-btn:hover { color: #f5f0e8 !important; }
        .reset-btn:hover { border-color: rgba(255,255,255,0.25) !important; color: #ccc !important; }
        .hist-card:hover { border-color: rgba(255,255,255,0.15) !important; }
        textarea:focus { border-color: rgba(255,180,60,0.5) !important; outline: none; }
      `}</style>
      ...
      <div style={S.container}>
        ...
        <div style={S.header}>
          ...
          <p style={S.subtitle}>Turn any overwhelming goal into tiny steps you can actually start.</p>
        </div>

        {/* Streak bar */}
        {appData.streak > 0 && (
          <div style={S.streakBar}>
            <span style={S.streakEmoji}>{streakEmoji(appData.streak)}</span>
            ...
            <div style={S.streakDots}>
              {Array.from({ length: Math.min(appData.streak, 7) }).map((_, i) => (
                <div key={i} style={{ ...S.dot, background: i < appData.streak ? "#ffb43c" : "rgba(255,255,255,0.1)" }} />
              ))}
            </div>
          </div>
        )}
          ...
          <div style={{ animation: "fadeUp 0.3s ease both" }}>
            <div style={S.inputCard}>
              ...
              <textarea
                ...
                placeholder={phFade ? placeholders[placeholderIdx] : ""}
                rows={3}
                onKeyDown={e => { if (e.key === "Enter" && e.metaKey) decompose(); }}
              />
              <div style={S.inputFooter}>
                <span style={S.hint}>⌘ + Enter to submit</span>
                <button
                  style={{ ...S.btn, ...(!goal.trim() || loading ? S.btnDisabled : {}) }}
                  onClick={decompose}
            ...
            {result && (
              <div style={{ animation: "fadeUp 0.4s ease both", ...(celebrateGoal ? { animation: "celebrate 3s ease both" } : {}) }}>
                ...
                {completedCount > 0 && (
                  <div style={{ marginBottom: "16px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                      ...
                      {allDone && <span style={{ color: "#9ee8cc", fontSize: "13px", animation: "pop 0.4s ease both" }}>You did it! 🎉</span>}
                    </div>
                  ...
                  {result.tasks.map((task, i) => (
                    ...
                    >
                      <div style={{ display: "flex", gap: "14px", alignItems: "flex-start", flex: 1 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <p style={{ ...S.taskAction, ...(checked[task.id] ? S.taskStrike : {}) }}>{task.action}</p>
          ...
          <div style={{ animation: "fadeUp 0.3s ease both" }}>
            {appData.history.length === 0 ? (
              <div style={S.emptyState}>
                ...
                <p style={{ color: "#666", fontSize: "15px" }}>No goals completed yet.</p>
                <p style={{ color: "#555", fontSize: "13px" }}>Complete your first goal to see it here!</p>
                <button style={{ ...S.btn, marginTop: "16px" }} onClick={() => setTab("decompose")}>
                  Break down a goal ✦
                </button>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                <div style={S.statsRow}>
                  <div style={S.statCard}>
                    <div style={S.statNum}>{appData.history.length}</div>
                    <div style={S.statLabel}>Goals completed</div>
                  </div>
                  <div style={S.statCard}>
                    <div style={{ ...S.statNum, color: "#ffb43c" }}>{appData.streak}</div>
                    <div style={S.statLabel}>Day streak {streakEmoji(appData.streak)}</div>
                  </div>
                  <div style={S.statCard}>
                    <div style={{ ...S.statNum, color: "#9ee8cc" }}>
                      {appData.history.reduce((a, h) => a + h.completedCount, 0)}
                    </div>
                    <div style={S.statLabel}>Tasks crushed</div>
                  </div>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
                        <div style={S.completeBadge}>✓ Done</div>
                      ...
                      <div style={{ marginTop: "14px", borderTop: "1px solid rgba(255,255,255,0.07)", paddingTop: "14px", animation: "fadeUp 0.25s ease both" }}>
                        ...
                        {entry.tasks.map(t => (
                          <div key={t.id} style={{ display: "flex", gap: "10px", alignItems: "flex-start", marginBottom: "8px" }}>
                            <span style={{ color: "#9ee8cc", fontSize: "13px", flexShrink: 0 }}>✓</span>
                ...
                <button
                  style={{ ...S.resetBtn, marginTop: "8px", color: "#e06060", borderColor: "rgba(224,96,96,0.2)" }}
                  onClick={() => {
                    if (window.confirm("Clear all history and streak?")) {
...
const S = {
  ...
  blob1: { position: "fixed", top: "-100px", left: "-80px", width: "420px", height: "420px", borderRadius: "50%", background: "radial-gradient(circle, rgba(255,180,60,0.1) 0%, transparent 70%)", pointerEvents: "none" },
  ...
  streakEmoji: { fontSize: "22px" },
  streakText: { flex: 1, fontSize: "14px" },
  streakDots: { display: "flex", gap: "5px" },
  ...
  inputCard: { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: "20px", padding: "26px", marginBottom: "20px" },
  ...
  btn: { background: "linear-gradient(135deg, #ffb43c, #ff8c42)", color: "#1a0e00", border: "none", borderRadius: "12px", padding: "11px 22px", fontSize: "15px", fontWeight: "700", cursor: "pointer", fontFamily: "'Georgia', serif", transition: "opacity 0.2s" },
  ...
  statNum: { fontSize: "28px", fontWeight: "900", color: "#f5f0e8", lineHeight: 1 },
  statLabel: { color: "#666", fontSize: "12px", marginTop: "5px" },
  ...
  histMeta: { color: "#666", fontSize: "12px", margin: 0 },
  completeBadge: { background: "rgba(100,220,180,0.1)", border: "1px solid rgba(100,220,180,0.2)", color: "#9ee8cc", fontSize: "11px", padding: "3px 10px", borderRadius: "20px" },
